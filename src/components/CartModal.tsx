...
const CartModal = () => {
  ...
  const handleWhatsApp = () => {
    const numero = "524793203429"; // 👈 Tu número aquí
        ...
        {items.length === 0 ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto space-y-4 py-4">
              {items.map((item) => (
                <div key={`${item.id}-${item.talla}-${item.color}-${item.grabado}`} className="flex gap-4 p-3 rounded-xl bg-muted/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 ml-auto text-destructive" onClick={() => removeItem(item.id, item.talla, item.color, item.grabado)}>
